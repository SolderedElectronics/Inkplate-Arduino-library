...
{   
    int16_t x = 0, y = 0;
    uint16_t w = 0, h = 0;
    display.getTextBounds(txt, 50, 0, &x, &y, &w, &h);
    ...
    while( *(text + cnt) != '\0')
    {
      display.print( *(text + cnt));
      if( display.getCursorX() > display.width()/2 + w / 2 - 3)
      {
        display.setCursor(display.width()/2 - w/2 - 3, display.height()/2 - h / 2 + 3 + 9 * row);
      }
      cnt++;
      row++;
    }
