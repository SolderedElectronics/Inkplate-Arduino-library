...
void drawNews(struct news *entities)
{
    ...
    for (int i = 0; i < maxBoxes && entities[i].title != NULL && entities[i].description != NULL; i++)
    {
        ...
        // Draw the title
        display.drawTextBox(leftMargin, y0, display.width() - rightMargin-230, y0 + 70,entities[i].title,1, &Inter16pt7b, 25, false, 12);

        // Draw the description
        display.drawTextBox(leftMargin, y0 + 65, display.width() - rightMargin-250, y1,entities[i].description, 1, &Inter12pt7b, 23, false, 10);
    }
}
