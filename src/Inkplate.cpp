...
Inkplate::Inkplate()
    : Adafruit_GFX(E_INK_WIDTH, E_INK_HEIGHT),
      Graphics(E_INK_WIDTH, E_INK_HEIGHT)
#else

Inkplate::Inkplate(uint8_t _mode)
    : Adafruit_GFX(E_INK_WIDTH, E_INK_HEIGHT),
      Graphics(E_INK_WIDTH, E_INK_HEIGHT)
#endif
{
  ...
  for (uint32_t i = 0; i < 256; ++i)
    pinLUT[i] = ((i & B00000011) << 4) | (((i & B00001100) >> 2) << 18) |
 ...
 */
void Inkplate::preloadScreen() { memcpy(DMemoryNew, _partial, 60000); }
