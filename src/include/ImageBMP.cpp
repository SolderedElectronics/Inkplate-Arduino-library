 ...
 */
bool Image::drawBitmapFromSd(const char *fileName, int x, int y, bool dither,
                             bool invert) {
  SdFile dat;
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawBitmapFromSd(SdFile *p, int x, int y, bool dither,
                             bool invert) {
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawBitmapFromWeb(const char *url, int x, int y, bool dither,
                              bool invert) {
  bool ret = 0;
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawBitmapFromWeb(WiFiClient *s, int x, int y, int32_t len,
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawBitmapFromBuffer(uint8_t *buf, int x, int y, bool dither,
                                 bool invert) {
  bitmapHeader bmpHeader;
 ...
 *              1 if using invert, 0 if not
 */
void Image::displayBmpLine(int16_t x, int16_t y, bitmapHeader *bmpHeader,
                           bool dither, bool invert) {
  ...
  for (int j = 0; j < w; ++j) {
    ...
    case 4: {
      uint8_t px =
          pixelBuffer[j >> 1] & (j & 1 ? 0x0F : 0xF0) >> (j & 1 ? 0 : 4);
      uint8_t val;
    ...
    case 16: {
      uint16_t px =
          ((uint16_t)pixelBuffer[(j << 1) | 1] << 8) | pixelBuffer[(j << 1)];
      ...
      if (dither)
#ifdef ARDUINO_INKPLATECOLOR
        val = ditherGetPixelBmp(((uint32_t)r << 16) | ((uint32_t)g << 8) |
                                    ((uint32_t)b),
                                j, y, w, 0);
#else
...
#ifdef ARDUINO_INKPLATECOLOR
        val = findClosestPalette(((uint32_t)r << 16) | ((uint32_t)g << 8) |
                                 ((uint32_t)b));
#else
        val = RGB3BIT(r, g, b);
#endif
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawBmpFromWebAtPosition(const char *url, const Position &position,
  ...
  uint16_t posX, posY;
  getPointsForPosition(position, bmpHeader.width, bmpHeader.height, E_INK_WIDTH,
                       E_INK_HEIGHT, &posX, &posY);
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawBmpFromSdAtPosition(const char *fileName,
                                    const Position &position, const bool dither,
                                    const bool invert) {
  SdFile dat;
  if (!dat.open(fileName, O_RDONLY))
    return 0;
  ...
  for (int i = 0; i < h; ++i) {
    ...
    displayBmpLine(posX, posY + bmpHeader.height - i - 1, &bmpHeader, dither,
                   invert);
  }
