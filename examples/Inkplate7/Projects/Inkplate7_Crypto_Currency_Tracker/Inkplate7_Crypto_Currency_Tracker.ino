/*
   Inkplate7_Crypto_Currency_Tracker example for Soldered Inkplate 7
...
void drawGraph()
{
    ...
    for (int i = 0; i < 31; ++i)
    {
        ...
        if (i)
        {
            display.drawThickLine(prev_x, prev_y, tx, ty, INKPLATE7_BLACK, 2);
        }
    ...
    // Draw grid
    for (int i = 0; i < 4; ++i)
        display.drawFastHLine(x1, y2 + i * (y1 - y2) / 4, x2 - x1, INKPLATE7_RED);
    for (int i = 0; i < 5; ++i)
        display.drawFastVLine(x1 + i * (x2 - x1) / 5, y2, y1 - y2, INKPLATE7_RED);

    display.drawFastVLine(x2 - textMargin + 2, y2, y1 - y2, INKPLATE7_RED);
    display.drawThickLine(x1, y1, x2, y1, INKPLATE7_RED, 2);
}
