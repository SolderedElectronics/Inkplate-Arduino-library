...
void Graphics::writePixel(int16_t x0, int16_t y0, uint16_t _color) {
  ...
  *(DMemory4Bit + E_INK_WIDTH / 2 * y0 + _x) =
      (pixelMaskGLUT[_x_sub] & temp) | (_x_sub ? _color : _color << 4);
}
 ...
 */
bool Inkplate::getPanelDeepSleepState() { return _panelState; }
