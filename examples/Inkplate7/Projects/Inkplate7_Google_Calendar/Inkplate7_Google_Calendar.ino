/*
   Inkplate7_Google_Calendar example for Soldered Inkplate 7
   ...
   This project shows you how Inkplate 7 can be used to display
...
#define DELAY_SECS 60 * 4 // Every 4 minutes
...
void drawGrid()
{
    ...
    for (int i = 0; i < m + 1; ++i)
    {
        ...
        display.setCursor(35 + (int)((float)x1 + (float)i * (float)(x2 - x1) / (float)m) + 15, y1 + header - 12);
        display.println(temp);
    }
}
...
bool drawEvent(entry *event, int day, int beginY, int maxHeigth, int *heigthNeeded)
{
    ...
    int lastSpace = -100;
    display.setCursor(x1 + 9 - day, beginY + 9);
    ...
    // Set cursor to print time and/or location
    display.setCursor(x1 + 47 - day, display.getCursorY() + 10);
    display.setFont();
    ...
    *heigthNeeded = display.getCursorY() + 14 - y1;
    ...
    return display.getCursorY() < maxHeigth - 15;
}
...
void drawData()
{
    ...
    for (int i = 0; i < entriesNum; ++i)
    {
        ...
        int shift = 0;
        bool s = drawEvent(&entries[i], entries[i].day, columns[entries[i].day] + 48, E_INK_HEIGHT - 15, &shift);
