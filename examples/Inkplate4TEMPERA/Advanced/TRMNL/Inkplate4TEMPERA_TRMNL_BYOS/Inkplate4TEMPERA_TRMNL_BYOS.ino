...
// WAKE button (same pin as the Inkplate4TEMPERA_Wake_Up_Button example): pressing
...
void setup()
{
    ...
    // depress the voltage reported to the server. Inkplate 4 TEMPERA reads
    // it from the onboard BQ27441 fuel gauge, which reports millivolts and
    // which display.begin() leaves powered down - wake it just long enough
    // for the reading, then put it back to sleep so it doesn't add its
    // 30-50 uA draw during deep sleep.
    display.wakePeripheral(INKPLATE_FUEL_GAUGE);
    display.battery.begin();
    batteryVoltage = display.battery.voltage() / 1000.0;
...
void doDisplay()
{
    ...
    http.addHeader("Model", "inkplate_4tempera");
