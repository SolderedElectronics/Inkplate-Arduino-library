...
void pngle_on_draw(pngle_t *pngle, uint32_t x, uint32_t y, uint32_t w,
  ...
  if (_pngPosition != Image::_npos) {
    _imagePtrPng->getPointsForPosition(_pngPosition, pngle_get_width(pngle),
                                       pngle_get_height(pngle), E_INK_WIDTH,
  ...
  if (rgba[3])
    for (int j = 0; j < h; ++j)
      for (int i = 0; i < w; ++i) {
        ...
        uint8_t px =
            _imagePtrPng->findClosestPalette((r << 16) | (g << 8) | (b));
#else
...
#ifdef ARDUINO_INKPLATECOLOR
          px =
              _imagePtrPng->ditherGetPixelBmp((r << 16) | (g << 8) | (b), x + i,
                                              y + j, _imagePtrPng->width(), 0);
#else
 ...
 */
bool Image::drawPngFromSd(const char *fileName, int x, int y, bool dither,
                          bool invert) {
  SdFile dat;
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawPngFromWeb(const char *url, int x, int y, bool dither,
                           bool invert) {
  _pngDither = dither;
  _pngInvert = invert;
  lastY = y;
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawPngFromWeb(WiFiClient *s, int x, int y, int32_t len,
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawPngFromWebAtPosition(const char *url, const Position &position,
 ...
 * @return      1 if drawn successfully, 0 if not
 */
bool Image::drawPngFromSdAtPosition(const char *fileName,
