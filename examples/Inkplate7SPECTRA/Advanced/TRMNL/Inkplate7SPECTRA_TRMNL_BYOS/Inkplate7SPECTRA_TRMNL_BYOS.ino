/**
 **************************************************
 * @file        Inkplate7SPECTRA_TRMNL_BYOS.ino
 * @brief       TRMNL BYOS client example for Soldered Inkplate 7SPECTRA.
 *
 * @details     Connects Inkplate 7SPECTRA to WiFi, registers with a
 ...
 *
 *              NOTE: Inkplate 7SPECTRA is a 6-color panel and has no fast
 ...
 *
 * Button refresh: pressing the WAKE button (GPIO18) while the device is
...
// WAKE button (same pin as the Inkplate7SPECTRA_Wake_Up_Button example): pressing
    ...
    while (WiFi.status() != WL_CONNECTED)
    {
        if (millis() - start > WIFI_TIMEOUT_MS)
        {
            if (showBanner)
            {
                ...
                // This panel has no partial refresh, so a one-off full refresh
                // is acceptable here before the long backoff sleep.
                display.display();
            }
...
void doDisplay()
{
    ...
    http.addHeader("Model", "inkplate_7spectra");
