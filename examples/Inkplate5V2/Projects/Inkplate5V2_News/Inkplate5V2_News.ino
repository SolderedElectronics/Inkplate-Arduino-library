...
Network network;
Inkplate display(INKPLATE_1BIT); // Use 1-bit grayscale mode for Inkplate5
...
void drawNews(struct news *entities)
{
    ...
    // Render news items
    int startY = 160; // Adjust starting Y position for news items
    int boxHeight = 130; // Reduce box height for smaller screen
    ...
    for (int i = 0; i < maxBoxes && entities[i].title != NULL && entities[i].description != NULL; i++)
    {
        ...
        // Draw the title
        display.drawTextBox(leftMargin, y0, display.width() - rightMargin-120, y0 + 70,entities[i].title, 1, &Inter16pt7b, 26, false, 12);

        // Draw the description
        display.drawTextBox(leftMargin, y0 + 65, display.width() - rightMargin-140, y1,entities[i].description, 1, &Inter12pt7b, 20, false, 10);
    }
}
void setTime()
{
