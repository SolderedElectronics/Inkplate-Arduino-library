...
uint8_t Image::ditherGetPixelBmp(uint32_t px, int i, int j, int w,
  ...
  uint8_t oldPixel =
      min((uint16_t)0xFF, (uint16_t)((uint16_t)ditherBuffer[0][i] + px));

  uint8_t newPixel =
      oldPixel & (getDisplayMode() == INKPLATE_1BIT ? B10000000 : B11100000);
...
uint8_t Image::ditherGetPixelJpeg(uint8_t px, int i, int j, int x, int y, int w,
  ...
  uint16_t oldPixel =
      min((uint16_t)0xFF,
          (uint16_t)((uint16_t)px + (uint16_t)jpegDitherBuffer[j + 1][i + 1] +
