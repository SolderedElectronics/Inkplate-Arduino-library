 ...
 * header plus telemetry the server can show (WiFi RSSI, model, panel
 * resolution, and what woke the device). It draws the returned image and
 * deep-sleeps for the refresh_rate the server specifies. If the server is
 * still serving the image that's already on the panel, the sketch skips both
 * the download and the costly e-paper refresh, and the banner is not
 * repainted either - the current image stays intact between wakes.
 *
 ...
 * WIFI_RETRY_TIMES / API_RETRY_TIMES below). Since Inkplate 2 has no wake
 * button, persistent WiFi failures settle into retrying once an hour; API
 * errors settle into a 15-minute polling fallback. A 202/500 "not ready"
 * status from the server is re-polled after a minute.
 *
...
// Inkplate 2 has no wake button, so wakes are timer-driven only.
...
// the device settles at WIFI_FALLBACK_SLEEP; once API_RETRY_TIMES is
// exhausted it settles at API_FALLBACK_SLEEP.
const int WIFI_RETRY_TIMES[] = {60, 180, 300};
const int API_RETRY_TIMES[] = {15, 30, 60};
#define API_FALLBACK_SLEEP 900
// no wake button on Inkplate 2, so keep retrying hourly instead of sleeping until a button press
#define WIFI_FALLBACK_SLEEP 3600
    ...
    while (WiFi.status() != WL_CONNECTED)
    {
        if (millis() - start > WIFI_TIMEOUT_MS)
        {
            if (showBanner)
            {
                ...
                // A one-off full refresh before a long backoff sleep is fine on this panel.
                display.display();
            }
...
void doDisplay()
{
    ...
    // Inkplate 2 cannot measure battery voltage, so the Battery-Voltage header is omitted.
    http.addHeader("RSSI", String(WiFi.RSSI()));
    http.addHeader("Model", "inkplate_2");
...
void wifiErrorSleep()
{
    ...
    if (wifiRetryCount < (int)(sizeof(WIFI_RETRY_TIMES) / sizeof(WIFI_RETRY_TIMES[0])))
        seconds = WIFI_RETRY_TIMES[wifiRetryCount++];
    else
        seconds = WIFI_FALLBACK_SLEEP;
    goToSleep(seconds);
}
